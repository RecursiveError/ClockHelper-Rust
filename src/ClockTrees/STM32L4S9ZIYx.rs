...
impl PLLSAI2QConf {
    pub fn get(&self) -> Result<f32, ClockError> {
        match self {
            PLLSAI2QConf::DIV2 => return Ok(2.0),
            PLLSAI2QConf::DIV4 => return Ok(4.0),
            PLLSAI2QConf::DIV6 => return Ok(6.0),
            PLLSAI2QConf::DIV8 => return Ok(8.0),
        }
    }
}
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl PLLSAI2RDIVIDERConf {
    pub fn get(&self) -> Result<f32, ClockError> {
        match self {
            PLLSAI2RDIVIDERConf::DIV2 => return Ok(2.0),
            PLLSAI2RDIVIDERConf::DIV4 => return Ok(4.0),
            PLLSAI2RDIVIDERConf::DIV8 => return Ok(8.0),
            PLLSAI2RDIVIDERConf::DIV16 => return Ok(16.0),
        }
    }
}
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub struct ClockTree {
    ...
    pub PLLSAI2Q: PLLSAI2QConf,
    pub PLLSAI2R: PLLSAI2RConf,
    pub PLLSAI2RDIVIDER: PLLSAI2RDIVIDERConf,
...
impl Default for ClockTree {
    fn default() -> Self {
        Self {
            ...
            PLLM1: PLLM1Conf::Value(1),
            PLLM2: PLLM2Conf::Value(1),
            PLLM3: PLLM3Conf::Value(1),
...
impl ClockTree {
    ...
    fn PLLM1_get(&self) -> Result<f32, ClockError> {
        let input = self.PLLSource_get()? as f32;
        let value = self.PLLM1.get()? as f32;
        Ok((input / value) as f32)
    }

    fn PLLM2_get(&self) -> Result<f32, ClockError> {
        let input = self.PLLSource_get()? as f32;
        let value = self.PLLM2.get()? as f32;
        Ok((input / value) as f32)
    }

    fn PLLM3_get(&self) -> Result<f32, ClockError> {
        let input = self.PLLSource_get()? as f32;
        let value = self.PLLM3.get()? as f32;
        Ok((input / value) as f32)
    }
    ...
    fn SDMMC1Mult_get(&self) -> Result<f32, ClockError> {
        match self.SDMMC1Mult {
            SDMMC1MultConf::PLLP => return self.PLLP_get(),
    ...
    fn DSIMult_get(&self) -> Result<f32, ClockError> {
        match self.DSIMult {
            DSIMultConf::PLLSAI2Q => return self.PLLSAI2Q_get(),
    ...
    fn PLLN_get(&self) -> Result<f32, ClockError> {
        let input = self.PLLM1_get()? as f32;
    ...
    fn PLLSAI1N_get(&self) -> Result<f32, ClockError> {
        let input = self.PLLM2_get()? as f32;
    ...
    fn PLLSAI2N_get(&self) -> Result<f32, ClockError> {
        let input = self.PLLM3_get()? as f32;
    ...
    fn PLLSAI2Q_get(&self) -> Result<f32, ClockError> {
        let input = self.PLLSAI2N_get()? as f32;
        let value = self.PLLSAI2Q.get()? as f32;
        Ok((input / value) as f32)
    }

    pub fn PLLSAI2Qoutput_get(&self) -> Result<f32, ClockError> {
        self.PLLSAI2Q_get()
    }
    ...
    fn PLLSAI2RDIVIDER_get(&self) -> Result<f32, ClockError> {
        let input = self.PLLSAI2R_get()? as f32;
        let value = self.PLLSAI2RDIVIDER.get()? as f32;
        Ok((input / value) as f32)
    }

    pub fn LTDCCLK_get(&self) -> Result<f32, ClockError> {
        let input = self.PLLSAI2RDIVIDER_get()?;
        if input > (120000000 as f32) {
            return Err(ClockError {
                err_type: ClockErrorType::Overflow(input as u32, 120000000),
                from: ClockNodes::PLLSAI2RDIVIDER,
                to: ClockNodes::LTDCCLK,
            });
        } else if input < (0 as f32) {
            return Err(ClockError {
                err_type: ClockErrorType::Underflow(input as u32, 0),
