...
impl ClockTree {
    ...
    fn LPUARTMult_get(&self) -> Result<f32, ClockError> {
        match self.LPUARTMult {
            ...
            LPUARTMultConf::Clk16MHzOutput => return self.Clk16MHzOutput_get(),
        };
    }
    pub fn ClkLPUARTOutput_get(&self) -> Result<f32, ClockError> {
        let input = self.LPUARTMult_get()?;
        if input > (32000000 as f32) {
            return Err(ClockError {
                err_type: ClockErrorType::Overflow(input as u32, 32000000),
                from: ClockNodes::LPUARTMult,
                to: ClockNodes::ClkLPUARTOutput,
            });
        } else if input < (0 as f32) {
            return Err(ClockError {
                err_type: ClockErrorType::Underflow(input as u32, 0),
    ...
    fn CLKI2S3Mult_get(&self) -> Result<f32, ClockError> {
        match self.CLKI2S3Mult {
            ...
            CLKI2S3MultConf::RC64MPLL => return self.RC64MPLL_get(),
        };
    }
